<template>
    <div class="inbox">
        <h2 class="header">All Emails</h2>
        <div v-if="allEmails.length === 0">
        ...
        <div v-else>
            ...
            <ul class="email-list">
                <li v-for="email in sortedEmails" :key="email.id" class="main-item">
                    <div v-if="email.type === 'inbox'" class="email-item" :class="{ 'unread': !email.read, 'read': email.read }">
                    ...
                    <div v-else>
                        <div class="email-item" :class="{read}">
                            <div class="email-details">
                                <div class="email-header">
                                    <div class="part_1">
                                        <div class="checkbox">
                                            ...
                                            <div @click="toggleStarred(email)" class="star">
                                                <i :class="['fa', email.important ? 'fa-star checked' : 'far fa-star']"></i>
                                                <!-- <i v-if="email.isImportant" class="fa fa-star checked"></i>
                                            <i v-else class="far fa-star"></i> -->
                                            </div>
                                        </div>
                                        <div class="email-sender">To: {{ email.recipientIds.join(', ') }}</div>
                                    </div>
                                    <div class="email-date">Date: {{ formatDate(email.sentDate) }}</div>
                                    <div class="part_2">
                                        <div class="folders" @mouseover="showFolders(email)"
                                            @mouseleave="hideFolders(email)">
                                            <i class="fas fa-folder-plus"></i>
                                            <div v-show="email.showFolders" class="folder-dropdown">
                                                <div v-for="folder in folders" :key="folder"
                                                    @click="copyToFolder(email, folder)">
...
export default {
    ...
    methods: {
        fetchEmails() {
            ...
            axios.get(`http://localhost:8080/users/get/${emailId}/sent`)
                .then((sentResponse) => {
                    // this.allEmails = response.data;
                    axios.get(`http://localhost:8080/users/get/${emailId}/inbox`)
                        .then((inboxResponse) => {
                            this.allEmails = sentResponse.data.concat(inboxResponse.data);
                            console.log('All emails:', this.allEmails);
                        })
                        .catch((inboxError) => {
                            console.error('Error fetching sent emails:', inboxError);
                        });
                })
                .catch((sentError) => {
                    console.error('Error fetching inbox emails:', sentError);
                });
        },
        emailView(email) {
    ...
    computed: {
        sortedEmails() {
            return this.allEmails.slice().sort((a, b) => new Date(b.sentDate) - new Date(a.sentDate));
        },
    },
    mounted() {
        this.fetchEmails();
